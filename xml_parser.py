...
class XMLParser:
    def __init__(self, xml_file_path, namespace=None) -> None:
        if namespace is None:
            ET.register_namespace(
                "", "http://www.sa.dk/xmlns/siard/1.0/schema0/table11.xsd"
            )
        else:
    ...
    def add_document_to_node(
        self, node: ET.Element, document_to_add: str
    ) -> None:
        tree = ET.parse(document_to_add)
        element_to_add = tree.getroot()
        node.append(element_to_add)

    def add_xml_string_to_node(
        self, node: ET.Element, xml_str_to_add: str
    ) -> None:
        root_element = ET.fromstring(xml_str_to_add)
        node.append(root_element)

    def add_element_to_node(
        self, node: ET.Element, element_to_add: ET.Element
    ) -> None:
        node.append(element_to_add)
